 ...
 * returns List[StateTransitionGQLModel]
 * where : StateTransitionWhereFilter
 * arguments
 * skip : 
 * limit : 
 * orderby : 
 * where : 
 */
export const StateSourcesQuery = `
query StateSources($id: UUID!, $skip: Int, $limit: Int, $where: StateTransitionWhereFilter) {
...
`
export const StateSourcesQueryAction = CreateAsyncActionFromQuery(StateSourcesQuery, {}, UpdateVectorAfterFetch("sources"))

export const StateSourcesLoadMoreButton = ({ state, limit=10, where, action=StateSourcesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...state, limit: _state.limit, skip: _state.skip, where: where}))
