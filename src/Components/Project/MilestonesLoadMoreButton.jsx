 ...
 * arguments
 */
export const ProjectMilestonesQuery = `
query ProjectMilestones($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const ProjectMilestonesQueryAction = CreateAsyncActionFromQuery(ProjectMilestonesQuery, {}, UpdateVectorAfterFetch("milestones"))

export const ProjectMilestonesLoadMoreButton = ({ project, limit=10, where, action=ProjectMilestonesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...project, limit: _state.limit, skip: _state.skip, where: where}))
