 ...
 * returns List[PublicationGQLModel]
 *  : 
 * arguments
 */
export const PublicationtypePublicationsQuery = `
query PublicationtypePublications($id: UUID!, $skip: Int, $limit: Int, $where: ) {
    result: publicationtypeById(id: $id) {
        id
        publications(skip: $skip, limit: $limit, where: $where) {
            __typename
            id
            name
            created
...
`
export const PublicationtypePublicationsQueryAction = CreateAsyncActionFromQuery(PublicationtypePublicationsQuery, {}, UpdateVectorAfterFetch("publications"))

export const PublicationtypePublicationsLoadMoreButton = ({ publicationtype, limit=10, where, action=PublicationtypePublicationsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...publicationtype, limit: _state.limit, skip: _state.skip, where: where}))
