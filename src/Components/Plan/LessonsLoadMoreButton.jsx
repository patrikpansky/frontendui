 ...
 * arguments
 */
export const PlanLessonsQuery = `
query PlanLessons($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const PlanLessonsQueryAction = CreateAsyncActionFromQuery(PlanLessonsQuery, {}, UpdateVectorAfterFetch("lessons"))

export const PlanLessonsLoadMoreButton = ({ plan, limit=10, where, action=PlanLessonsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...plan, limit: _state.limit, skip: _state.skip, where: where}))
