 ...
 * returns List[MilestoneGQLModel]
 *  : 
 * arguments
 */
export const MilestoneNextsQuery = `
query MilestoneNexts($id: UUID!, $skip: Int, $limit: Int, $where: ) {
    result: milestoneById(id: $id) {
        id
        nexts(skip: $skip, limit: $limit, where: $where) {
            id
            name
            startdate
            enddate
...
`
export const MilestoneNextsQueryAction = CreateAsyncActionFromQuery(MilestoneNextsQuery, {}, UpdateVectorAfterFetch("nexts"))

export const MilestoneNextsLoadMoreButton = ({ milestone, limit=10, where, action=MilestoneNextsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...milestone, limit: _state.limit, skip: _state.skip, where: where}))
