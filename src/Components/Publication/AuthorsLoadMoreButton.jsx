 ...
 * returns List[PublicationAuthorGQLModel]
 *  : 
 * arguments
 */
export const PublicationAuthorsQuery = `
query PublicationAuthors($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const PublicationAuthorsQueryAction = CreateAsyncActionFromQuery(PublicationAuthorsQuery, {}, UpdateVectorAfterFetch("authors"))

export const PublicationAuthorsLoadMoreButton = ({ publication, limit=10, where, action=PublicationAuthorsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...publication, limit: _state.limit, skip: _state.skip, where: where}))
