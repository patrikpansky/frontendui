 ...
 * returns List[RequestHistoryGQLModel]
 *  : 
 * arguments
 */
export const RequestHistoriesQuery = `
query RequestHistories($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const RequestHistoriesQueryAction = CreateAsyncActionFromQuery(RequestHistoriesQuery, {}, UpdateVectorAfterFetch("histories"))

export const RequestHistoriesLoadMoreButton = ({ request, limit=10, where, action=RequestHistoriesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...request, limit: _state.limit, skip: _state.skip, where: where}))
