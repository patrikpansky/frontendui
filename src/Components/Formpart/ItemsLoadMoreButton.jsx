 ...
 * returns List[FormItemGQLModel]
 *  : 
 * arguments
 */
export const FormpartItemsQuery = `
query FormpartItems($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const FormpartItemsQueryAction = CreateAsyncActionFromQuery(FormpartItemsQuery, {}, UpdateVectorAfterFetch("items"))

export const FormpartItemsLoadMoreButton = ({ formpart, limit=10, where, action=FormpartItemsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...formpart, limit: _state.limit, skip: _state.skip, where: where}))
