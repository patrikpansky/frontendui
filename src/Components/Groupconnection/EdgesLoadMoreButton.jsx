 ...
 * 
 * returns List[GroupGQLModelConnectionEdge]
 *  : 
 * arguments
 */
export const GroupconnectionEdgesQuery = `
query GroupconnectionEdges($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const GroupconnectionEdgesQueryAction = CreateAsyncActionFromQuery(GroupconnectionEdgesQuery, {}, UpdateVectorAfterFetch("edges"))

export const GroupconnectionEdgesLoadMoreButton = ({ groupconnection, limit=10, where, action=GroupconnectionEdgesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...groupconnection, limit: _state.limit, skip: _state.skip, where: where}))
