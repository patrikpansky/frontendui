 ...
 * returns List[AcProgramStudentGQLModel]
 * where : ProgramStudentInputFilter
 * arguments
 * skip : 
 * limit : 
 * where : 
 ...
 * desc : 
 */
export const AcprogramStudentsQuery = `
query AcprogramStudents($id: UUID!, $skip: Int, $limit: Int, $where: ProgramStudentInputFilter) {
...
`
export const AcprogramStudentsQueryAction = CreateAsyncActionFromQuery(AcprogramStudentsQuery, {}, UpdateVectorAfterFetch("students"))

export const AcprogramStudentsLoadMoreButton = ({ acprogram, limit=10, where, action=AcprogramStudentsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...acprogram, limit: _state.limit, skip: _state.skip, where: where}))
