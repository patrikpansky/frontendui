 ...
 * arguments
 */
export const AcsubjectPublicationQuery = `
query AcsubjectPublication($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const AcsubjectPublicationQueryAction = CreateAsyncActionFromQuery(AcsubjectPublicationQuery, {}, UpdateVectorAfterFetch("publication"))

export const AcsubjectPublicationLoadMoreButton = ({ acsubject, limit=10, where, action=AcsubjectPublicationQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...acsubject, limit: _state.limit, skip: _state.skip, where: where}))
