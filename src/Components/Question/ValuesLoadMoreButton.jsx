 ...
 * returns List[QuestionValueGQLModel]
 *  : 
 * arguments
 */
export const QuestionValuesQuery = `
query QuestionValues($id: UUID!, $skip: Int, $limit: Int, $where: ) {
    result: questionById(id: $id) {
        id
        values(skip: $skip, limit: $limit, where: $where) {
            __typename
            id
            name
            lastchange
...
`
export const QuestionValuesQueryAction = CreateAsyncActionFromQuery(QuestionValuesQuery, {}, UpdateVectorAfterFetch("values"))

export const QuestionValuesLoadMoreButton = ({ question, limit=10, where, action=QuestionValuesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...question, limit: _state.limit, skip: _state.skip, where: where}))
