 ...
 * returns List[FormItemTypeGQLModel]
 *  : 
 * arguments
 */
export const FormitemcategoryTypesQuery = `
query FormitemcategoryTypes($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const FormitemcategoryTypesQueryAction = CreateAsyncActionFromQuery(FormitemcategoryTypesQuery, {}, UpdateVectorAfterFetch("types"))

export const FormitemcategoryTypesLoadMoreButton = ({ formitemcategory, limit=10, where, action=FormitemcategoryTypesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...formitemcategory, limit: _state.limit, skip: _state.skip, where: where}))
