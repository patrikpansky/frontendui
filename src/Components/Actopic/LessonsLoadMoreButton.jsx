 ...
 * Actopic.lessons
 * Lessons for a topic
 * returns List[AcLessonGQLModel]
 *  : 
 * arguments
 */
export const ActopicLessonsQuery = `
query ActopicLessons($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const ActopicLessonsQueryAction = CreateAsyncActionFromQuery(ActopicLessonsQuery, {}, UpdateVectorAfterFetch("lessons"))

export const ActopicLessonsLoadMoreButton = ({ actopic, limit=10, where, action=ActopicLessonsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...actopic, limit: _state.limit, skip: _state.skip, where: where}))
