 ...
 * returns List[FinanceGQLModel]
 *  : 
 * arguments
 */
export const FinancetypeFinancesQuery = `
query FinancetypeFinances($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const FinancetypeFinancesQueryAction = CreateAsyncActionFromQuery(FinancetypeFinancesQuery, {}, UpdateVectorAfterFetch("finances"))

export const FinancetypeFinancesLoadMoreButton = ({ financetype, limit=10, where, action=FinancetypeFinancesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...financetype, limit: _state.limit, skip: _state.skip, where: where}))
