 ...
 * 
 * returns List[UserGQLModelConnectionEdge]
 *  : 
 * arguments
 */
export const UserconnectionEdgesQuery = `
query UserconnectionEdges($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const UserconnectionEdgesQueryAction = CreateAsyncActionFromQuery(UserconnectionEdgesQuery, {}, UpdateVectorAfterFetch("edges"))

export const UserconnectionEdgesLoadMoreButton = ({ userconnection, limit=10, where, action=UserconnectionEdgesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...userconnection, limit: _state.limit, skip: _state.skip, where: where}))
