 ...
 * Formtype.forms
 * 
 * returns List[FormGQLModel]
 *  : 
 * arguments
 */
export const FormtypeFormsQuery = `
query FormtypeForms($id: UUID!, $skip: Int, $limit: Int, $where: ) {
    result: formtypeById(id: $id) {
        id
        forms(skip: $skip, limit: $limit, where: $where) {
            id
            name
            lastchange
            created
...
`
export const FormtypeFormsQueryAction = CreateAsyncActionFromQuery(FormtypeFormsQuery, {}, UpdateVectorAfterFetch("forms"))

export const FormtypeFormsLoadMoreButton = ({ formtype, limit=10, where, action=FormtypeFormsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...formtype, limit: _state.limit, skip: _state.skip, where: where}))
