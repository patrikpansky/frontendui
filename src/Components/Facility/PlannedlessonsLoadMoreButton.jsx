 ...
 * arguments
 */
export const FacilityPlannedlessonsQuery = `
query FacilityPlannedlessons($id: UUID!, $skip: Int, $limit: Int, $where: ) {
...
`
export const FacilityPlannedlessonsQueryAction = CreateAsyncActionFromQuery(FacilityPlannedlessonsQuery, {}, UpdateVectorAfterFetch("plannedLessons"))

export const FacilityPlannedlessonsLoadMoreButton = ({ facility, limit=10, where, action=FacilityPlannedlessonsQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...facility, limit: _state.limit, skip: _state.skip, where: where}))
