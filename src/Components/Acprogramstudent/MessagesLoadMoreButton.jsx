 ...
 * returns List[AcProgramMessageGQLModel]
 * where : ProgramMessagesInputFilter
 * arguments
 * skip : 
 * limit : 
 * where : 
 ...
 * desc : 
 */
export const AcprogramstudentMessagesQuery = `
query AcprogramstudentMessages($id: UUID!, $skip: Int, $limit: Int, $where: ProgramMessagesInputFilter) {
...
`
export const AcprogramstudentMessagesQueryAction = CreateAsyncActionFromQuery(AcprogramstudentMessagesQuery, {}, UpdateVectorAfterFetch("messages"))

export const AcprogramstudentMessagesLoadMoreButton = ({ acprogramstudent, limit=10, where, action=AcprogramstudentMessagesQueryAction, children }) => {
    const [_state, _setState] = useState(
        { skip: 0, limit: limit, more: true, loading: false, errors: null }
    )
    const dispatch = useDispatch()
    const onLoadMore = async () => {
        const jsondata = await dispatch(action({...acprogramstudent, limit: _state.limit, skip: _state.skip, where: where}))
